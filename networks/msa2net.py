...
class MyDecoderLayerAdapLKAFreq(nn.Module):
    def __init__(
        ...
        self.layer_lka_1 = AdaptiveAttentionModule(in_channels=out_dim)
        ## Prompt Module must be located here.
...
class Msa2Net_V4(nn.Module):
    def __init__(self, num_classes=9):
        ...
        self.decoder_3 = MyDecoderLayerAdapLKAFreq(
            (d_base_feat_size, d_base_feat_size),
            in_out_chan[3],
            decoder_prompt=False,
            n_class=num_classes)

        self.decoder_2 = MyDecoderLayerAdapLKAFreq(
            (d_base_feat_size * 2, d_base_feat_size * 2),
            in_out_chan[2],
            decoder_prompt=False,
            n_class=num_classes)
        
        self.decoder_1 = MyDecoderLayerAdapLKAFreq(
            (d_base_feat_size * 4, d_base_feat_size * 4),
            in_out_chan[1],
            decoder_prompt=False,
            n_class=num_classes)
